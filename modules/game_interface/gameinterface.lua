...
local OPCODE_AREA_TEXT = 72
local areaTextLabel = nil
local areaTextHideEvent = nil
local areaTextFadeInEvent = nil
local areaTextFadeOutEvent = nil

local function cancelAreaTextEvents()
  if areaTextHideEvent then
    removeEvent(areaTextHideEvent)
    areaTextHideEvent = nil
  end
  if areaTextFadeInEvent then
    removeEvent(areaTextFadeInEvent)
    areaTextFadeInEvent = nil
  end
  if areaTextFadeOutEvent then
    removeEvent(areaTextFadeOutEvent)
    areaTextFadeOutEvent = nil
  end
end

local function fadeInAreaText(duration)
  if not areaTextLabel then return end

  cancelAreaTextEvents()

  local steps = 10
  local stepTime = math.floor(duration / steps)
  if stepTime < 1 then stepTime = 1 end

  local step = 0
  local startOpacity = 0
  local endOpacity = 1

  areaTextLabel:setOpacity(startOpacity)
  areaTextLabel:setVisible(true)

  local function doStep()
    if not areaTextLabel then
      areaTextFadeInEvent = nil
      return
    end
    ...
    step = step + 1
    local t = step / steps
    local newOpacity = startOpacity + (endOpacity - startOpacity) * t
    areaTextLabel:setOpacity(newOpacity)

    if step < steps then
      areaTextFadeInEvent = scheduleEvent(doStep, stepTime)
    else
      areaTextFadeInEvent = nil
      areaTextLabel:setOpacity(endOpacity)
    end
  end
...
local function fadeOutAreaText(duration)
  if not areaTextLabel then return end

  -- If already invisible, just bail
  if areaTextLabel:getOpacity() <= 0 then
    areaTextLabel:setVisible(false)
    return
  end
  ...
  local step = 0
  local startOpacity = areaTextLabel:getOpacity()
  if startOpacity <= 0 then startOpacity = 1 end
  local endOpacity = 0
  ...
  local function doStep()
    ...
    if step < steps then
      areaTextFadeOutEvent = scheduleEvent(doStep, stepTime)
    else
      areaTextFadeOutEvent = nil
      areaTextLabel:setOpacity(endOpacity)
      areaTextLabel:setVisible(false)
...
local function hideAreaText()
  if not areaTextLabel then return end
  cancelAreaTextEvents()
  areaTextLabel:setOpacity(0)
  areaTextLabel:setVisible(false)
end

local function showAreaText(text, ms)
  if not areaTextLabel then
    return
  end
  ...
  -- how long the fade should be (max half the total time, capped at 600ms)
  local fadeDuration = math.min(600, math.floor(ms / 2))

  cancelAreaTextEvents()

  areaTextLabel:setText(text)

  -- Fade in
  fadeInAreaText(fadeDuration)

  -- Schedule fade out a bit before the text lifetime ends
  local fadeOutDelay = ms - fadeDuration
  if fadeOutDelay < 0 then fadeOutDelay = 0 end

  areaTextHideEvent = scheduleEvent(function()
    fadeOutAreaText(fadeDuration)
  end, fadeOutDelay)
end


local function onAreaOpcode(protocol, opcode, buffer)
    if not buffer or buffer == "" then
        return
    end
    -- Debug:
    print("[areatext] extended opcode:", opcode, buffer)
    showAreaText(buffer, 4000)
end
...
function init()
    ...
    gameLeftLockPanel = gameRootPanel:recursiveGetChildById('leftLock')
        ...
        -- Area text label
    areaTextLabel = gameRootPanel:getChildById('areaTextLabel')
    if not areaTextLabel then
        print("[areatext] areaTextLabel not found in gameRootPanel")
    end

    -- Extended opcode for area text
    if ProtocolGame and ProtocolGame.registerExtendedOpcode then
        ProtocolGame.registerExtendedOpcode(OPCODE_AREA_TEXT, onAreaOpcode)
    else
        print("[areatext] WARNING: ProtocolGame.registerExtendedOpcode missing")
    end
...
function terminate()
    ...
    if ProtocolGame and ProtocolGame.unregisterExtendedOpcode then
        ProtocolGame.unregisterExtendedOpcode(OPCODE_AREA_TEXT)
    end
    if areaTextHideEvent then
        removeEvent(areaTextHideEvent)
        areaTextHideEvent = nil
...
function onGameEnd()
        -- Hide area text when leaving game
    hideAreaText()
    hide()
end
